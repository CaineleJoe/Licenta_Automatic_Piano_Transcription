...
def process_file(args):
    ...
    try:
        ...
        print(f"ERRROR at {audio_path}: {e}")
    return results
...
def main():
    ...
    print(f"A total of {num_processes} parallel processes are being used.")
    ...
    for split in splits:
        ...
        with tf.io.TFRecordWriter(output_tfrecord_path) as writer:
            with multiprocessing.Pool(processes=num_processes) as pool:
                progress_bar = tqdm(pool.imap_unordered(process_file, tasks), total=len(tasks), desc=f"Processing {split}")
